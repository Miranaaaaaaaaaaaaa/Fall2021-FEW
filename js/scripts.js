//DS EDIT - Created a variable for opacity on stroke 
...
jQuery(function ($) {
	...
	$(document).ready(function() {
		...
		//Convert modula galleries into clickable things
		//jQuery('.ds-project-gallery .modula-item').each(function (index) {jQuery(this).click( function () {window.smooth_scrollbar.scrollIntoView(document.getElementById("ds-project-id-" + (index+1)))})});
		//jQuery('.ds-project-gallery .modula-item').each(function (index) {jQuery(this).click( function () {document.getElementById("ds-project-id-" + (index+1)).scrollIntoView()})});


		//Make more parts of the lightbox clikcable to close
		jQuery(document).bind('modula_fancybox_lightbox_after_show',function () {jQuery(".modula-fancybox-stage").click(function () {jQuery('.modula-fancybox-button--close').click();});});
		//Clear img titles
		jQuery(document).bind('modula_fancybox_lightbox_after_show',function () { jQuery('img[title]').each(function() { $(this).removeAttr('title'); })});this
	});
...
---------------------------------------------------*/
	...
	function PageLoad() {	
		...
		// BEGIN OLD ARCHIVED TIME SECTION
		...
		$( document ).ready(() => {
								...
								if( $('#hero').hasClass("has-image")) {	
									...
									TweenMax.to($(".hero-title"), 0.4, {force3D:true, y: 10, opacity:1, delay:0, ease:Power2.easeOut});
									TweenMax.to($(".hero-subtitle"), 0.4, {force3D:true, y:10, opacity:1, delay:0, ease:Power2.easeOut});
									...
									TweenMax.to($(".hb-right"), 0.4, {force3D:true, y:0, opacity:1, delay:0, ease:Power2.easeOut});									
									TweenMax.to($("#main-page-content"), 0.4, {force3D:true, opacity:1, y:0, delay:.3, ease:Power2.easeOut});  //delay was 1.15
								} else {
									TweenMax.to($(".hero-title"), 0.4, {force3D:true, y: 10, opacity:1, delay:0.3, ease:Power2.easeOut}); 
									TweenMax.to($(".hero-subtitle"), 0.4, {force3D:true, y: 10, opacity:1, delay:0.3, ease:Power2.easeOut});
								}
		...
		$('body').waitForImages({
						finished: function() {
							...
							TweenMax.to($(".percentage"),0.3, {force3D:true, opacity:0, y:-30, delay:0, ease:Power1.easeInOut});
							TweenMax.to($(".headphones, .headphones-text"),0.1, {force3D:true, opacity:0, y:-50, delay:0.0, ease:Power2.easeOut});
							TweenMax.to($(".preloader-wrap"),0.4, {force3D:true, yPercent: -101, delay:0.0, ease:Power2.easeInOut}); //delay was .6
							TweenMax.set($(".preloader-wrap"), {visibility:'hidden', delay:0, opacity:0}); // delay was 1.4
							...
							setTimeout(function(){
								...
								if( $('#hero').hasClass("has-image")) {	
									...
									TweenMax.to($(".hero-title"), 0.4, {force3D:true, y: 10, opacity:1, delay:0.3, ease:Power2.easeOut}); 
									TweenMax.to($(".hero-subtitle"), 0.4, {force3D:true, y: 10, opacity:1, delay:0.3, ease:Power2.easeOut});									
									TweenMax.to($("#main-page-content"), 0.4, {force3D:true, opacity:1, y:0, delay:0.3, ease:Power2.easeOut});							
								}								
								...
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().find('.title'), 0.5, {force3D:true, opacity:strokeOpacity, y:-slidertitleheight, delay:0.8, ease:Power2.easeOut});
								...
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').find('.title'), 0.5, {force3D:true, opacity:0, y:0, delay:0.9, ease:Power2.easeOut});
								//DS EDIT - SO LIST ISN'T VISIBLE on first load
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().find('.title'), 0.0, {force3D:true, opacity:0, y:slidertitleheight, delay:1, ease:Power2.easeOut});
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().find('.title'), 0.0, {force3D:true, opacity:0, y:slidertitleheight, delay:1, ease:Power2.easeOut});
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().find('.title'), 0.0, {force3D:true, opacity:0, y:slidertitleheight*2, delay:1, ease:Power2.easeOut});
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().next().find('.title'), 0.0, {force3D:true, opacity:0, y:slidertitleheight*3, delay:1, ease:Power2.easeOut});
								TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().next().next().find('.title'), 0.0, {force3D:true, opacity:0, y:slidertitleheight*4, delay:1, ease:Power2.easeOut});
		...
		//hard fix modula slider galleries
		//
		// find any video-sizer, descendent of a slick-track
		// observe height of sibling
		// if sibling's height changes, change video sizer height
		
		$('.slick-slide .video-sizer').each( (index,el) => {
			console.log('found sizer' + el.HTML);
			var modItem = el.parentElement;
			var parentSlide = modItem.parentElement.parentElement;
			var slideIndex = parseInt(parentSlide.dataset.slickIndex);
			console.log('index:' + slideIndex);
			
			if (slideIndex != 0) {
				console.log('setting first child');
				var newHeight = parentSlide.parentElement.children[0].getBoundingClientRect().height;
			} else {
				console.log('setting 2nd  child');
				var newHeight = parentSlide.parentElement.children[1].getBoundingClientRect().height;
			}
				console.log('new height: ' + newHeight);
				modItem.style.height = "" + newHeight + "px";
		});
...
---------------------------------------------------*/	
	...
	function FirstLoad() {	
		...
		$('.button-wrap.menu, #burger-wrapper').on('click', function() {
		...
		$('.quickmenu, #close-quickmenu').on('click', function() {			
			...
			setTimeout( function(){			
				if ($('#quickmenu').hasClass("active")) {	
					...
					TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().find('.title'), 0.0, {force3D:true, opacity:0, y:-slidertitleheight*2, delay:0.0, ease:Power2.easeOut});
					...
					TweenMax.to($('.stroked .swiper-pagination-bullet-active').find('.title'), 0.0, {force3D:true, opacity:0, y:-slidertitleheight/2, delay:0.0, ease:Power2.easeOut});
					TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().find('.title'), 0.0, {force3D:true, opacity:0, y:slidertitleheight, delay:0.0, ease:Power2.easeOut});
					
					TweenMax.to($("#footer-container"), 0.3, {force3D:true, opacity:0, delay:0.4, ease:Power2.easeOut});
					
					TweenMax.set($('.stroked .swiper-pagination-bullet-active').prev().find('.title'),0.0, {opacity:0, y:-slidertitleheight, delay:0.0});
					TweenMax.set($('.no-stroked .swiper-pagination-bullet-active').find('.title'), 0.0,{opacity:0, y:slidertitleheight/2, delay:0.0});
					TweenMax.set($('.stroked .swiper-pagination-bullet-active').find('.title'),0.0, {opacity:0, y:slidertitleheight/2, delay:0.0});
					TweenMax.set($('.stroked .swiper-pagination-bullet-active').next().find('.title'), 0.0,{opacity:0, y:slidertitleheight*2, delay:0.0});
...
---------------------------------------------------*/
	...
	function Showcase() {
		...
		if( $('#showcase-slider').length > 0 ){	
			...
			var swiperOptions = {
				...
				pagination: {
					...
					renderBullet: function (index, className) {
						if (index == 0) {
							return '<div class="tab__link ' + className + '">' + '<div class="subtitle">' + "NYU/TISCH DESIGN FOR STAGE AND FILM" + '</div>' + '<div class="title">' + titles[index] + '<span class="counter">' + counter[index] + '</span>' + '</div>' + '</div>';
						} else {
							return '<div class="tab__link ' + className + '">' + '<div class="subtitle">'  + subtitle[index] + " - " + index + " / 18 " + '</div>' + '<div class="title">' + titles[index] + '<span class="counter">' + counter[index] + '</span>' + '</div>' + '</div>';
						}
				...
				on: {
					...
					init: function () {
						...
						$('.showcase-captions-wrap').clone().removeClass('no-stroked').addClass('stroked').insertAfter('.showcase-captions-wrap').css('opacity','0.0');
						$('.stroked .showcase-captions').clone().removeClass('no-stroked').appendTo('.showcase-subtitles-wrap');
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').find('.title'), 0, {force3D:true, opacity:0, y:slidertitleheight, delay:0, ease:Power2.easeOut});
					...
					slideChangeTransitionStart: function () {
						...
						if (window.dsSwiper.activeIndex == 0) {
							TweenMax.to($('.showcase-captions-wrap.stroked'), 0.3, { opacity:0, ease:Power1.easeInOut});
						} else {
							TweenMax.to($('.showcase-captions-wrap.stroked'), 0.6, { opacity:1, ease:Power1.easeInOut});
						}

						TweenMax.to('.showcase-captions-wrap.no-stroked', 1,{opacity: 1, ease:Power1.easeOut});
						//DS EDIT - Created a variable for opacity on stroke 
						TweenMax.to($('.no-stroked .swiper-pagination-bullet-active').prev().prev().prev().find('.title'), 0.6, {y:-slidertitleheight*3, opacity:1, ease:Power1.easeInOut});
						TweenMax.to($('.no-stroked .swiper-pagination-bullet-active').prev().prev().find('.title'), 0.6, {y:-slidertitleheight*2, opacity:1, ease:Power1.easeInOut});
						...
						TweenMax.to($('.no-stroked .swiper-pagination-bullet-active').next().next().find('.title'), 0.6, {y:slidertitleheight*2, opacity:1, ease:Power1.easeInOut})
						TweenMax.to($('.no-stroked .swiper-pagination-bullet-active').next().next().next().find('.title'), 0.6, {y:slidertitleheight*3, opacity:1, ease:Power1.easeInOut})
						
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().find('.title'), 0.6, {y:-slidertitleheight, opacity:strokeOpacity, ease:Power1.easeInOut})
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().prev().find('.title'), 0.6, {y:-slidertitleheight*2, opacity:strokeOpacity, ease:Power1.easeInOut})						
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().prev().prev().find('.title'), 0.6, {y:-slidertitleheight*3, opacity:strokeOpacity, ease:Power1.easeInOut})
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().prev().prev().prev().find('.title'), 0.6, {y:-slidertitleheight*4, opacity:0, ease:Power1.easeInOut})
						
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').find('.title'), 0.6, {y:0, opacity:1, delay:0, ease:Power1.easeInOut})											
						
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().find('.title'), 0.6, {y:slidertitleheight, opacity:strokeOpacity, ease:Power1.easeInOut})
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().find('.title'), 0.6, {y:slidertitleheight*2, opacity:strokeOpacity, ease:Power1.easeInOut})
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().next().find('.title'), 0.6, {y:slidertitleheight*3, opacity:strokeOpacity, ease:Power1.easeInOut})
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().next().next().find('.title'), 0.6, {y:slidertitleheight*4, opacity:strokeOpacity, ease:Power1.easeInOut})
						TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().next().next().next().next().find('.title'), 0.6, {y:slidertitleheight*5, opacity:0, ease:Power1.easeInOut})
						
						TweenMax.to($('.showcase-subtitles-wrap .swiper-pagination-bullet-active').prev().find('.subtitle'), 0.6, {y:-50, opacity:1, ease:Power1.easeInOut})
						...
						TweenMax.to($('.showcase-subtitles-wrap .swiper-pagination-bullet-active').next().find('.subtitle'), 0.6, {y:50, opacity:1, ease:Power1.easeInOut})									
			...
			//DS EDIT - Turning off built-in nav
			console.log('removing default URL');
			$('.swiper-slide-active a.title').removeAttr('href');
			$('.swiper-slide-active a.title').unbind();
			
			$( document ).ready( () => { 
					console.log('Prepping video');
					if ($('.bgvid')[0].readyState > 0) 
					{
						
						console.log('Its ready');
						TweenMax.to('.ds-playbutton', .5,{opacity: 1, ease:Power1.easeOut});
						$('.ds-playbutton').css('visibility','visible');
						
					} else {
						console.log('setting an event');
						$('.bgvid')[0].addEventListener('loadedmetadata',() => {
							console.log('metadata happened-a');
							TweenMax.to('.ds-playbutton', .5,{opacity: 1, ease:Power1.easeOut})});
							$('.ds-playbutton').css('visibility','visible');
						$('.bgvid')[0].addEventListener('canplay',() => {
							console.log('canplay happened-a');
							TweenMax.to('.ds-playbutton', .5,{opacity: 1,  ease:Power1.easeOut})});
							$('.ds-playbutton').css('visibility','visible');
					}
			});
			...
			$('.ds-playbutton').click(() => {
				var myVideo = $('.bgvid')[0];
					console.log("videostate: " + myVideo.readyState);
					console.log("videoerror: " + myVideo.error);
					$('.bgvid').css('opacity','1');
					myVideo.play(); 
					...
					$('.ds-playbutton').css('opacity','0.0');
					$('.ds-playbutton').css('visibility','hidden');
					$('.showcase-captions-wrap.no-stroked').css('opacity','0.0');
					$('.ds-subbuttons').css('opacity','1.0');
				    $('.ds-subbuttons').css('visibility','visible');

				//register end event.

				myVideo.addEventListener('ended', () => {swiper.slideNext()});

			 });

			 $('.ds-playpausebutton').click(() => {
				var myVideo = $('.bgvid')[0];
				if (myVideo.paused) {
					myVideo.play(); 
					TweenMax.to('.showcase-captions-wrap.no-stroked', 1,{opacity: 0, ease:Power1.easeOut});
				} else { 
					myVideo.pause();
					TweenMax.to('.showcase-captions-wrap.no-stroked', 1,{opacity: 1, ease:Power1.easeOut});
				}
			 ...
			 $('.ds-mutebutton').click( () => {
				var myVideo = $('.bgvid')[0]; 
				if (myVideo.muted) {
					$('.ds-mutebutton i').removeClass('fa-volume-mute').addClass('fa-volume-up');
					myVideo.muted = false;
				} else {
					$('.ds-mutebutton i').removeClass('fa-volume-up').addClass('fa-volume-mute');
					myVideo.muted = true;	
				}
			});
			
			//DS EDIT - Created a variable for opacity on stroke 
			//TweenMax.to($('.stroked .swiper-pagination-bullet-active').prev().find('.title'), 0.5, {force3D:true, opacity:strokeOpacity, y:-slidertitleheight, delay:0.4, ease:Power2.easeOut});
			...
			//TweenMax.to($('.stroked .swiper-pagination-bullet-active').find('.title'), 0.5, {force3D:true, opacity:strokeOpacity, y:0, delay:0.5, ease:Power2.easeOut});
			//TweenMax.to($('.stroked .swiper-pagination-bullet-active').next().find('.title'), 0.5, {force3D:true, opacity:strokeOpacity, y:slidertitleheight, delay:0.6, ease:Power2.easeOut});
